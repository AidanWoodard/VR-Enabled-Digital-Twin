...
**Always tell the user explicitly before handing work to a subagent** (which agent, what task, and that it starts without this conversation's context unless it's a fork). Never switch agents silently mid-task.

**Audit repo state before committing, and proactively surface cleanup opportunities.** Multiple sessions (and worktrees under `.claude/worktrees/`) work on this repo in parallel, so before staging anything: check `git status`, `git worktree list`, and `git stash list`; expect that another session may have advanced master since the conversation started. When a worktree/branch/stash looks stale, verify it's truly superseded with evidence (ancestor check via `git merge-base --is-ancestor`, or byte-level diff of its working files against current master — commit history alone isn't proof either way), then report findings and offer the removal rather than deleting silently. Keep recorded bag data (`recordings/`, `playback_captures/`, `dashboard_bags/`) out of commits.
...
**`arm_bag_recorder.py`** — MUX node multiplexing the arm's `FollowJointTrajectory` action server between live VR teleop (`/sgr532/ik_goal_cmd`) and `rosbag`-based playback. Driven by string commands on `/sgr532/bag_control` (`RECORD:N`, `PLAY:N`, `STOP`, `CLEAR:N`) — published by `dashboard_controller.py` (Unity's dashboard UI) or manually via `rostopic pub`. Publishes latched status on `/sgr532/bag_status` (`IDLE`/`RECORDING:N`/`PLAYING:N`/`PLAYING_EE:N`). Records `/sgr532/joint_states` + `/sgr532/gripper/command` + `/sgr532/vr_target_pose` to `~/ROS_Files/sagittarius_ws/recordings/slot_N.bag`. Recordings are finalized with SIGINT (so rosbag renames `.bag.active` → `.bag`), a `rospy.on_shutdown` handler SIGINTs any child rosbag on node shutdown, and playback falls back to a crash-orphaned `.bag.active` if no finalized `.bag` exists (formerly the SIGTERM bug that lost all MUX recordings — see `docs/RECORDING_PIPELINE_KNOWN_ISSUES.md`). Playback has two modes via the `~playback_mode` param (`joints` default, `ee` — set by `full_system_ee.launch`): joints mode replays recorded joint states; EE mode re-solves IK per recorded `vr_target_pose` (peeked via `rosbag play --immediate -u 5 -d 1`, since `-r 0` can miss the sparse first pose). Both modes **verified end-to-end on hardware 2026-07-15** (EE mode proven by replaying a bag with `joint_states` stripped via `rosbag filter` — arm still tracked, so only IK could be driving it). In joints mode `_cmd_play()` pre-positions the arm to the bag's first joint frame (peeked via a throwaway `rosbag play -r 0` onto the dedicated `PEEK_JS_TOPIC` — it must NOT reuse `PLAYBACK_JS_TOPIC`, because `_control_callback` holds `_state_lock` during the peek and `_playback_js_callback` on the same topic blocks rospy's sequential callback dispatch, starving the peek into a guaranteed timeout; `vr_target_pose`/`gripper` are remapped to void topics so nothing leaks to `light_ik_solver` or the gripper) **before** starting real-speed playback — this prevents the arm from visibly lurching/chasing the trajectory for the first 1-2s. All arm goals are built by `_make_goal()`, which always emits 2-point trajectories `[current joints @ t=0, target @ t=duration]` — required by the vendor driver (see "Vendor Driver Quirk" below). Peek/playback subprocesses are stopped with SIGINT, never `terminate()`, so `rosbag` finalizes bags and unregisters from the master (no zombie `/play_*` nodes). During the real-speed portion of every `PLAY` (both modes, after pre-positioning), the node also captures live `/sgr532/joint_states` to a CSV at `recordings/playbacks/slot_N_{joints|ee}_<timestamp>.csv` (`time,joint1..joint6`) via `_start_capture()`/`_stop_capture()` — no subprocess involved, since it just writes rows from the existing `_real_js_callback`. Capture stops on natural playback end or manual `STOP`, and `rospy.on_shutdown` closes any open capture file. Never overwrites (timestamped filenames), and is purely diagnostic — nothing reads these CSVs back into the arm's control path.

**`compare_playback.py`** — Standalone offline script (not a ROS node, not in `CMakeLists.txt`, run directly with `python3`). `python3 compare_playback.py <slot>` reads the original `recordings/slot_N.bag` (via the `rosbag` Python API — the only piece of this pipeline still touching bags) plus the latest `recordings/playbacks/slot_N_joints_*.csv` and `slot_N_ee_*.csv`, plots all 6 joints overlaying reference vs. each playback mode, saves a PNG to `recordings/playbacks/`, and prints per-joint + overall RMSE/max-error (rad).
...
## Vendor Driver Quirk: 2-Point Trajectory Minimum

`sdk_sagittarius_arm_real.cpp` **silently rejects** any `FollowJointTrajectory` goal with fewer than 2 trajectory points — and reports it as *succeeded* with `INVALID_GOAL` buried in the result, so the sender never sees an error and the arm simply doesn't move. Every goal must be `[current joints @ t=0, target @ t=duration]` (see `light_ik_solver.py`'s builder and `arm_bag_recorder._make_goal()`). This burned the entire MUX playback feature in 2026-07: all its single-point goals were dropped without a trace.

Related log noise: `move_group` prints "Empty Joint State" warnings on every `/sgr532/compute_ik` call because neither `light_ik_solver.py` nor the MUX seeds `ik_request.robot_state`. Benign — IK still succeeds; it appears during live teleop and EE playback alike.
